...
export class OTPService {
  ...
  async sendOTP(inputs): Promise<Record<string, any>> {
    ...
    const whereBuilder = `u.email = '${input}' OR  u.phone_no = '${input}'`;
    ...
    if (doesUserExists && inputs.email) {
      mobile = `${doesUserExists.callingCode}${doesUserExists.phone_no}`;
    }
  ...
  async verifyOTP(inputs): Promise<Record<string, any>> {
    ...
    if (!loginAttempt) {
      ...
      const {
        ...
        .post(this.config.get('services.api.loginWithMobile.url'), {
          // what is in 8080 port
          token: mobileToken,
